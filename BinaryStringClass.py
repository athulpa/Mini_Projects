...
# Displays the object in various formats (specified by Format argument).
    def disp(self, Format='string', FnOut = False, PrnOut = True):
...
# hash(b) combines b's uInt value and length into a tuple 't' and returns hash(t).
    ...
    def __str__(self):
        return self.disp(Format='string', FnOut=True, PrnOut=False)
    
    def __repr__(self):
        return 'BitString(' + self.disp(Format='debug', FnOut=True, PrnOut=False) + ')'
